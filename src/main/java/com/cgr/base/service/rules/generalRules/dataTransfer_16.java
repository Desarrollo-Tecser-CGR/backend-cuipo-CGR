...
public class dataTransfer_16 {
    ...
    public void applyGeneralRule16() {
        ...
        UtilsDB.ensureColumnsExist("GENERAL_RULES_DATA",
                ...
                "VAL_CompOblig_1erTrim_Cta2_16A:NVARCHAR(50)",
        ...
        String updateFirstTrimQuery = String.format(
                """
                        UPDATE d
                        SET
                            d.VAL_CompOblig_1erTrim_Cta2_16A = CASE
                                WHEN g.AMBITO_CODIGO IN ('A447', 'A448', 'A449', 'A450', 'A451', 'A453', 'A454')
                                    THEN CAST(g.OBLIGACIONES_SUM AS NVARCHAR(50))
                                ELSE CAST(g.COMPROMISOS_SUM AS NVARCHAR(50))
                            END
                        FROM GENERAL_RULES_DATA d
                        LEFT JOIN (
                            SELECT FECHA, CODIGO_ENTIDAD_INT, AMBITO_CODIGO,
                                   SUM(TRY_CAST(OBLIGACIONES AS DECIMAL(18,2))) AS OBLIGACIONES_SUM,
                                   SUM(TRY_CAST(COMPROMISOS AS DECIMAL(18,2))) AS COMPROMISOS_SUM
                            FROM %s
                            WHERE CUENTA = '2' AND TRIMESTRE = 3
                            GROUP BY FECHA, CODIGO_ENTIDAD_INT, AMBITO_CODIGO
                        ) g ON g.FECHA = d.FECHA
                            AND g.CODIGO_ENTIDAD_INT = d.CODIGO_ENTIDAD
                            AND g.AMBITO_CODIGO = d.AMBITO_CODIGO
                        """, TABLA_EJEC_GASTOS);

        jdbcTemplate.update(updateFirstTrimQuery);
                ...
                SET
                    VAL_Variacion_Monetaria_Cta2_16A =
                        CAST(
                            ...
                            - TRY_CAST(VAL_CompOblig_1erTrim_Cta2_16A AS DECIMAL(18,2))
                            AS NVARCHAR(50)
                        ),
                    VAL_Variacion_Porcentual_Cta2_16A =
                        CASE
                            WHEN TRY_CAST(VAL_CompOblig_1erTrim_Cta2_16A AS DECIMAL(18,2)) = 0 OR VAL_CompOblig_1erTrim_Cta2_16A IS NULL
                                THEN NULL
                            ELSE
                                FORMAT (
                                    ROUND(
                                        ((TRY_CAST(VAL_CompOblig_TrimVal_Cta2_16A AS DECIMAL(18,2))
                                        / NULLIF(TRY_CAST(VAL_CompOblig_1erTrim_Cta2_16A AS DECIMAL(18,2)), 0)) - 1) * 100, 2
                                    ),
                                    'N2'
                                )
                        END
