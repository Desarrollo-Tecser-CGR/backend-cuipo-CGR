        ...
        public void generateControlTable() {
                ...
                String sqlInsertData = """
                                ...
                                SELECT
                                    ...
                                    s.CODIGO_ENTIDAD,
                                    (
                                        SELECT TOP 1 s2.NOMBRE_ENTIDAD
                                        FROM SPECIFIC_RULES_DATA s2
                                        WHERE s2.FECHA = s.FECHA AND s2.CODIGO_ENTIDAD = s.CODIGO_ENTIDAD
                                    ) AS NOMBRE_ENTIDAD
                                FROM (
                                    SELECT DISTINCT s.FECHA, s.CODIGO_ENTIDAD
                ...
                int totalCalidad = ((Number) entityManager.createNativeQuery(totalCalidadQuery).getSingleResult())
                                .intValue();
                ...
                updateCalidadBuilder.append("WHEN (")
                                ...
                                .append(String.format(") AND %d = %d THEN 100.0 ", columnasGenerales.size(),
                                                totalCalidad));

                updateCalidadBuilder.append("ELSE CAST((")
                                .append(columnasGenerales.stream()
                                                .map(col -> String.format("CASE WHEN g.%s = 'CUMPLE' THEN 1 ELSE 0 END",
                                                                col))
                                ...
                                .append(" - (")
                                .append(columnasGenerales.stream()
                                                .map(col -> String.format(
                                                                "CASE WHEN g.%s = 'NO APLICA' THEN 1 ELSE 0 END", col))
                                                .collect(Collectors.joining(" + ")))
                ...
                updateL617Builder.append("WHEN (")
                                .append(columnasEspecificas.stream()
                                                ...
                                                .collect(Collectors.joining(" AND ")))
                                .append(String.format(") AND %d = %d THEN 100.0 ", columnasEspecificas.size(),
                                                totalL617));

                updateL617Builder.append("ELSE CAST((")
                                .append(columnasEspecificas.stream()
                                                .map(col -> String.format(
                                                                "CASE WHEN s.%s IN ('CUMPLE', 'NO EXCEDE') THEN 1 ELSE 0 END",
                                                                col))
                                                .collect(Collectors.joining(" + ")))
                                .append(") AS FLOAT) / NULLIF((")
                                .append(totalL617)
                                ...
                                .append(columnasEspecificas.stream()
                                                .map(col -> String.format(
                                                                "CASE WHEN s.%s = 'NO APLICA' THEN 1 ELSE 0 END", col))
                                                .collect(Collectors.joining(" + ")))
                                .append(")), 0) * 100 END AS porcentaje ");
