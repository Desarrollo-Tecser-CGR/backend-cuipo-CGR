...
public class dataTransfer_EG {
        ...
        String updateQuery = String.format("""
            WITH GASTOS_25 AS (
                SELECT
                    ...
                    CODIGO_ENTIDAD,
                    AMBITO_CODIGO,
                    SUM(CAST(COMPROMISOS AS FLOAT)) AS GASTOS_FUNCIONAMIENTO
                FROM %s
                WHERE 
                    (
                        (AMBITO_CODIGO = 'A438'
            ...
            SET
                ...
                r.GASTOS_FUNCIONAMIENTO = CAST(g.GASTOS_FUNCIONAMIENTO AS VARCHAR(MAX))
            FROM %s r
            JOIN GASTOS_25 g
               ON  r.FECHA          = g.FECHA
    ...
    public void applyGeneralRule26() {
        ...
        // 2. Construir la consulta con el WITH antes del INSERT
        //    Observa que ya no se pone "INSERT INTO ... WITH ..."
        //    sino "WITH ... INSERT INTO ..."
