...
public class queryFilters {
    ...
    private List<FormularioDTO> getFormTables() {
        String sql = String.format("SELECT DISTINCT [CODIGO_TABLA], [NOMBRE_TABLA] FROM [%s] ORDER BY [CODIGO_TABLA]",
                "general_rules_tables");
    ...
    private List<String> obtenerColumnasReglaGeneral(String codigoFormulario) {
        ...
        String sql = String.format("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ? AND (%s)", condition);
        return jdbcTemplate.queryForList(sql, String.class, tablaReglas);
    }
